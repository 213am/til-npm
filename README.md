...
- JavaScript 키 : `REACT_APP_KKO_MAP_KEY`
- `kakao developers - 내 애플리케이션 - 카카오맵` 활성화 하기

## 3. 카카오 지도 가이드

- [지도 가이드](https://apis.map.kakao.com/web/guide/)
...
<html lang="ko">
  <head>
    ...
    <script
      type="text/javascript"
      src="//dapi.kakao.com/v2/maps/sdk.js?appkey=REACT_APP_KKO_MAP_KEY"
    ></script>
    <title>카카오 지도</title>
  </head>
  <body>
...
function App() {
  // 앱 실행 후 컴포넌트가 마운트 될 때, js 를 실행한다
  // html 완료가 되면 출력
  useEffect(() => {
...
- 모든 API 키
  : 구글, 네이버, 카카오 관련 키들
  : Firebase, Supabase 등등...

- 네이밍 규칙
  : `REACT_APP_이름` 혹은 `VITE_이름` 를 프로젝트 생성 방식에 맞춰 접두어를 붙여서 작성
  : 예) 카카오 js 키(`REACT_APP_KKO_MAP_KEY`)
  : 예) 카카오 js 키(`VITE_KKO_MAP_KEY`)

- 이름에 담길 값도 `=` 다음 문자열로 작성
  : `" "` 는 생략
...
<html lang="en">
  <head>
    ...
    <script
      type="text/javascript"
      src="//dapi.kakao.com/v2/maps/sdk.js?appkey=%REACT_APP_KKO_MAP_KEY%&autoload=false&libraries=services"
    ></script>
  </head>
  <body>
    <div id="root"></div>
...
- [참고용 블로그](https://velog.io/@wlwl99/React-Kakao-Map-SDK-%EC%82%AC%EC%9A%A9%ED%95%98%EA%B8%B0)
